...
export function MarketCard({ index, filter }: MarketCardProps) {
  ...
  // Get the shares balance (updated function name)
  const { data: sharesBalanceData } = useReadContract({
    ...
    params: account?.address ? [BigInt(index), account.address] : undefined,
        ...
        <>
          ...
          <CardFooter>
            {market && sharesBalance && (
              <MarketSharesDisplay market={market} sharesBalance={sharesBalance} />
            )}
          </CardFooter>
        </>
      )}
